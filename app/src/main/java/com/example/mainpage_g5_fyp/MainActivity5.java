                    ...
                    RootRef.child("Users").child(phoneno).updateChildren(userdataMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                Toast.makeText(MainActivity5.this, "Congratulations! Account Created", Toast.LENGTH_SHORT).show();

                                Intent intent = new Intent(MainActivity5.this, MainActivity4.class);
                                startActivity(intent);
                            }
                            else {
                                Toast.makeText(MainActivity5.this, "Network Issue, try again later", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } else {
                    Toast.makeText(MainActivity5.this, "This number already created an account!",Toast.LENGTH_SHORT).show();
                }
            }
